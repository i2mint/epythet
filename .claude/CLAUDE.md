...
- **Seams are keyword arguments**: `api_generator`, `theme`, `agent_outputs` / `aggregates`, normalizer `rules`, `scaffold(pages=)`, `validate(backend=, ledger=)`, `ai_artifacts_template` (and `EPYTHET_AI_ARTIFACTS=0` as the fleet-wide off switch), `provenance` (`true` / `"minimal"` / `false`; `EPYTHET_PYPI_CHECK=0` skips the network lookup).
