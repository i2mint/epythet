...
- **Seams are keyword arguments**: `api_generator`, `theme`, `agent_outputs` / `aggregates`, normalizer `rules`, `scaffold(pages=)`, `validate(backend=, ledger=)`, `ai_artifacts_template`.
...
python -m doctest epythet/ai_artifacts.py epythet/themes.py epythet/normalizer.py
