...
`gh skill` needs GitHub CLI 2.90 or newer. Offline, or from the wheel, link the package's bundled copy:
...
Overriding the template: set `ai_artifacts_template = "docsrc/_templates/ai-agents.md"` (any project-relative path) to a `str.format` template using the fields `{marker}`, `{name}`, `{display_name}`, `{repo_stub}`, `{site_url}`, `{skills_section}`, `{subagents_section}`, `{instructions_section}`, `{outputs_section}`. A hand-written `docsrc/ai-agents.md` without the `<!-- generated by epythet -->` marker is never overwritten. `ai_artifacts = false` turns the page off. From Python: `epythet.ai_artifacts.render_ai_artifacts_page(artifacts, config, template=...)`.
