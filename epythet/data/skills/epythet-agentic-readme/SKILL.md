...
2. **Branch on the policy.**
   ...
   - `agentic_aspects = "add"`: follow the instruction snippet, which the user may have rewritten:
     ...
     The section goes right after the README's intro, before the second heading (the first one after the title), when `agentic_first` is true; at the end otherwise. A later `--write` updates it in place wherever the author moved it, taking its heading level from the heading that follows it. The check runs again after the write and its table is the report (in `--format json`, the `write` key carries the outcome). `--write` refuses, with a message and exit 2, rather than guess: a README with unpaired markers, a non-UTF-8 or non-Markdown README, a user template that does not start and end with the markers, a broken `pyproject.toml`, and a project with nothing agentic to document.
