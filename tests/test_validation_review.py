...
def test_select_pages_modes():
    ...
    assert select_pages(pages, mode="sample", sample=3) == [
        "index",
        "_autosummary/pkg",
        "_autosummary/pkg.mod",
    ]
...
def test_packet_layout_and_manifest(outdirs, data_dir):
    ...
    assert (
        packet.path / "pages" / "_autosummary" / "pkg.mod.txt"
    ).read_text() == "mod\n"
    manifest = json.loads((packet.path / "packet.json").read_text())
    assert (
        manifest["pages"] == packet.pages
        and manifest["prompt_hash"] == packet.prompt_hash
    )
    rubric = (packet.path / "rubric.md").read_text()
    assert (
        "DR001: RST field list leaked into prose" in rubric
        and "| A. Summary |" in rubric
    )
    assert json.loads((data_dir / "review" / "pkg" / "latest.json").read_text())[
        "packet"
    ] == str(packet.path)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            lambda r: r["proposed_rules"][0].pop("example_good"),
            "missing required key 'example_good'",
        ),
    ],
)
...
def test_review_level_writes_packet_and_ingests_reply(outdirs, data_dir, tmp_path):
    ...
    findings, notes = run_review_level(
        package="pkg", package_version=None, outdirs=outdirs, ledger=ledger, mode="all"
    )
    assert (
        [f.rule for f in findings] == [PACKET_RULE]
        and findings[0].level == 3
    ...
    assert (
        findings[1].fix
        and findings[1].tool == "review:test-model"
        and findings[1].detector == "llm"
    )
...
def test_review_never_gates_unless_asked():
    ...
    assert (
        report.exit_code("info", fail_on_review=True) == 0
    )  # a packet is not a defect
...
def test_propose_writes_a_loadable_proposed_rule_that_does_not_run_by_default(
    tmp_path, data_dir
):
    reply_path = tmp_path / "review.json"
...
def test_propose_rejects_a_rule_whose_examples_do_not_behave(tmp_path):
    reply = _reply()
    reply["proposed_rules"][0]["example_good"] = (
