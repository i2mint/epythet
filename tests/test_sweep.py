...
def test_sweep_is_read_only_and_ranks_the_bad_package_first(
    two_projects, tmp_path, data_dir
):
    bad, good = two_projects
    before = {**_mtimes(bad), **_mtimes(good)}
    result = sweep(
        [bad, good, tmp_path / "missing"], observations_path=tmp_path / "obs.jsonl"
    )
    ...
    assert (
        bad_counts["DR001"] == 1
        and bad_counts["DR003"] == 1
        and bad_counts["DQ002"] == 1
    )
...
def test_distribution_rows_are_complete(two_projects, tmp_path, data_dir):
    ...
    assert (
        row["severity"] == "error"
        and row["title"] == "RST field list leaked into prose"
    )
    assert row["per_100_objects"] == round(
        100 / sum(p.objects for p in result.swept), 2
    )


def test_queue_score_weights_entry_points_first():
    assert queue_score({"DQ002": 1}, severities={}) > queue_score(
        {"DQ003": 3}, severities={}
    )
...
def test_render_and_cli_json(two_projects, tmp_path, data_dir, capsys):
    ...
    assert (
        text.startswith("epythet sweep: 2 package(s) swept") and "queue (top 20" in text
    )
    assert "DR001     error" in text
    out = tmp_path / "sweep.json"
    sweep_command(
        str(bad), str(good), no_observe=True, format="json", output=str(out), quiet=True
    )
